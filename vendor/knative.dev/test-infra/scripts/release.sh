...
function tag_images_in_yamls_acr() {
  ...
  local SRC_DIR="${GOPATH}/src/"
  echo "Tagging any images under '${KO_DOCKER_REPO}' with ${XTAG}"
  for file in $@; do
    [[ "${file##*.}" != "yaml" ]] && continue
    echo "Inspecting ${file}"
    for image in $(grep -o "${KO_DOCKER_REPO}/[0-9a-z\./-]*@sha256:[0-9a-f]*" ${file}); do
      local dest_image=$(echo ${image} | cut -d"/" -f2 | cut -d"@" -f1)
      echo "Tagging ${dest_image}:${XTAG}"
      echo "az acr import -n ${AZ_ACR_NAME} --source ${image} -t ${dest_image}:${XTAG}"
      az acr import -n ${AZ_ACR_NAME} --source ${image} -t ${dest_image}:${XTAG}
    done
  done
}
